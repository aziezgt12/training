...
class C_Informasi extends BaseController
{
	...
	public function index()
	{
		...
		$data['title'] = "List OF Informasi Publik";

		$data['list'] = $this->db->get('tb_informasi')->result();
	...
	function save()
	{
		try {
			...
			$insert_data = [
				'jenis_informasi' => $param['jenis_informasi'],
				'judul_informasi' => $param['judul_informasi'],
				'link' => $file_name,
			];

			$this->db->insert('tb_informasi', $insert_data);
	...
	function delete()
	{
		...
		$sql = $this->db->delete('tb_informasi');
		if ($sql) {
